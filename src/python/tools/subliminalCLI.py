...
import tempfile #引入临时目录
import random
DEFAULT_CACHE_FILE = os.path.normpath(os.path.join(tempfile.gettempdir(), str(random.randint(10, 50)) + 'subliminal.cache.dbm'))

print("DEFAULT_CACHE_FILE = %s" % DEFAULT_CACHE_FILE)

def getLanguageAll():
    languageList = [{'alpha3':iso_language.alpha3, 'name': iso_language.name}
                    for iso_language in babelfish.language.LANGUAGE_MATRIX]
    return languageList
...
def subliminalCLI():
    ...
    output_exclusive_group.add_argument('-o', '--output_path', default='', help='use output_path')
    ...
    try:
        ...
        print("[cache] This region is already configured")
    ...
    # guess videos
    extend_videos = [Video.fromguess(os.path.split(p)[1], guessit.guess_file_info(p, 'autodetect')) for p in args.paths
                   if not os.path.exists(p)]
    videos.extend(extend_videos)
    ...
    # result output
    subtitles_count = 0
    videoNameList = [video.name for video in videos]
    if not subtitles:
        if not args.quiet:
            sys.stderr.write('No subtitles downloaded\n')
        return {'subtitles_count':subtitles_count,
